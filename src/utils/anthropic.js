...
- 文末表現の重複禁止：以下の表現を組み合わせて使い、同じものを2文以上連続させない
  　〜がみられます ／ 〜が確認されました ／ 〜と思われます ／ 〜と考えられます
  　〜傾向がうかがえます ／ 〜様子がうかがえます ／ 〜対応いたしました ／ 〜行いました
  　〜てまいります ／ 〜てまいりたいと思います
- 語尾パターンは最低3種類以上を使い分ける
- 「〜てまいります」の連続禁止：2文連続で使わず「〜していきます」「〜に努めます」「〜する予定です」に言い換える
- 冗長表現の禁止：「〜の一助となっている様子でした」→「〜となっているようでした」のように簡潔にする
- 一文一義：一つの文に主語の切り替えを含めない。主語が変わる場合は文を分ける
- 同語反復の禁止：「みられる」「対応する」「配慮する」など同じ動詞・表現を近い文脈で繰り返さない
- リズムのある文章：短文・中文・長文を混在させ、単調にならないようにする
- 全文を書き終えた後、重複表現がないか確認し、あれば修正してから出力する
...
- 語尾パターンは最低3種類以上を使い分ける
- 「〜てまいります」の連続禁止：2文連続で使わず「〜していきます」「〜に努めます」「〜する予定です」に言い換える
- 条件表現の明確化：「〜に留意しながら」より「〜がみられた際には〜」のように具体的な条件で書く
- 冗長表現を簡潔に：「〜の一助となっている様子でした」→「〜となっているようでした」
- 同語反復の禁止：「みられる」「対応する」「配慮する」など同じ動詞・表現を近い文脈で繰り返さない
- 一文一義：主語が変わる場合は文を分ける
- 全文を書き終えた後、重複表現がないか確認し、あれば修正してから出力する
...
- 語尾パターンは最低3種類以上を使い分ける
- 「〜てまいります」の連続禁止：2文連続で使わず「〜していきます」「〜に努めます」「〜する予定です」に言い換える
- 同語反復の禁止：「みられる」「対応する」「配慮する」など同じ動詞・表現を近い文脈で繰り返さない。言い換えや類義語を使う
- 冗長表現を避ける：「〜の一助となっている様子でした」→「〜となっているようでした」のように簡潔に
- 一文一義：主語が変わる場合は文を分ける。一つの文に複数の主語を入れない
- 読みやすいリズム：短文・中文を混在させ、単調にならないようにする
- 全文を書き終えた後、重複表現がないか確認し、あれば修正してから出力する
...
${hospitalizationNote ? `- 入院情報が含まれる場合：
  ...
  　・現在入院継続中の場合、締め文は「現在も入院中のため施術を休止しております」のように事実のみ記述すること。再開予定や主治医の判断などの表現は使わないこと
  　・退院・施術再開済みの場合、「〇月〇日より施術を再開しております」のように再開日を明記すること` : ''}
...
- 文末表現の重複禁止：以下の表現を組み合わせて使い、同じものを2文以上連続させない
  ...
  　〜てまいります ／ 〜てまいりたいと思います
- 語尾パターンは最低3種類以上を使い分ける
- 同語反復の禁止：「みられる」「対応する」「配慮する」など同じ動詞・言い回しを近い文脈で繰り返さない。言い換えを使う
- 「〜てまいります」の連続禁止：「継続してまいります」「行ってまいります」など〜てまいりますを2文連続させない
- 条件表現の明確化：「〜に留意しながら」→「〜がみられた際には」のように具体的な条件で表現する
- 冗長表現を避ける：「〜の一助となっている様子でした」→「〜となっているようでした」のように簡潔に
- 一文一義：主語が変わる場合は文を分ける
- 全文を書き終えた後、重複表現がないか自分で確認し、あれば修正してから出力する`;
